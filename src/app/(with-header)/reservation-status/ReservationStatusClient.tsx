...
export default function ReservationStatusClient() {
  ...
  const { data: dashboardData = [] } = useQuery({
    queryKey: ['reservation-dashboard', selectedExperience, currentDate.getFullYear(), currentDate.getMonth() + 1],
    queryFn: () => {
      ...
      return fetchReservationDashboard(Number(selectedExperience), yearStr, monthStr);
    },
  ...
  useEffect(() => {
    if (activitiesData?.activities && activitiesData.activities.length > 0 && !selectedExperience) {
